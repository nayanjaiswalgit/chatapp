...
window.addEventListener('popstate', ()=> dispatch({ type: "SHOWCHAT", payload: false }));
  const fatchdata = () => {
  ...
  return (
        ...
        <div className="-mt-1">
         ...
         {onstatus ? <div className="flex mx-3">
          ...
          <div className="flex ">
            ...
            <p className="text-white opacity-90  text-xs m-auto font-light text-center">Lastseen - {Timestamp(ontime.seconds)}
            </p>
          </div>}
        </div>
   ...
   { showuser && userdata?.photoURL && <UserProfile setshowuser={setshowuser} userdata={userdata}></UserProfile>}
    </div>
  )
}
