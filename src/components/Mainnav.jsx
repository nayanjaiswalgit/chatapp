...
function Mainnav() {
  const { setShowBackButton } = useContext(ChatContext);
  ...
  const  fatchdata = () => {
    const unsub = onSnapshot(doc(db, "userData", data.user.uid), (doc) => {
  ...
  if (Object.keys(data?.user).length === 0) {
    return (
      <div className=" w-full lg:mx-0 md:mx-0 ">
        <div className="lg:h-14 h-16  w-full text-white  border-l-2 rounded-md  lg:rounded-none md:rounded-none border-violet-700 bg-violet-800 flex justify-around  lg:pl-5 lg:pr-5 items-center relative text-4xl ">
          Welcome Back
        </div>
      </div>
    );
  ...
  return (
    <div className=" relative w-full lg:mx-0 md:mx-0  ">
      <div className="  lg:hidden p-2 bg-[url('.././src/img/chat.gif')] bg-cover bg-white ">
        {data?.chat && (
          <BiArrowBack
            className="lg:invisible md:invisible text-blue-500 absolute text-4xl left-5 top-1 bg-white rounded-full"
            onClick={() => {
              data.chat &&  setShowBackButton (false)}}
          />
        )}
      ...
      <div className="lg:h-14 h-16   w-full text-white border-l-2 rounded-md  lg:rounded-none md:rounded-none border-violet-700 bg-violet-800 flex justify-around lg:justify-between lg:pl-5 lg:pr-5 items-center relative">
        ...
        <div className="-mt-1">
          <h2 className="text-xl leading-7 text-white mx-auto font-Tilefont ">
          ...
          {onstatus ? (
            ...
            <div className="flex ">
              <p className="text-white opacity-90  text-xs m-auto font-light text-center">
                Lastseen - {Timestamp(ontime.seconds)}
              </p>
            </div>
          )}
        </div>
        <div className="  flex group  md:flex justify-between relative md:gap-5 lg:gap-5 items-center">
          <p className="absolute invisible group-hover:visible text-xs left-0 -top-5  opacity-50 ">
            Comming Soon..
          </p>
